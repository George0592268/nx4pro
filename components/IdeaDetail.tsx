...
import { 
  ...
  Rocket, Activity, Command, ExternalLink, Globe, CheckCircle2, ListChecks
} from 'lucide-react';
...
export const IdeaDetail: React.FC<Props> = ({ idea, onBack, onUpdateIdea, onGenerateLanding }) => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center min-h-full py-40 text-center animate-slide-up px-6">
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-12 space-y-12 pb-40 animate-slide-up">
      {/* Header */}
      <div className="flex items-center justify-between">
          <button onClick={onBack} className="flex items-center gap-2 opacity-40 hover:opacity-100 font-bold uppercase text-[9px] tracking-widest transition-all">
            <ArrowLeft size={12} /> <span>К списку идей</span>
          </button>
      ...
      {/* Main Title Hero */}
      <div className="bg-white dark:bg-white/5 rounded-5xl border border-black/5 dark:border-white/10 p-8 md:p-10 shadow-xl overflow-hidden relative group">
          ...
          <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                  ...
                  <div>
                      ...
                      <h2 className="text-base font-bold heading-refined opacity-80">{idea.title}</h2>
                  </div>
              </div>
              <h1 className="text-2xl md:text-3xl font-bold heading-refined leading-tight tracking-tight mb-4 max-w-3xl">{idea.problemStatement}</h1>
              <p className="opacity-40 text-xs italic font-medium">Прогноз окупаемости: {idea.roiEstimate}</p>
          </div>
      </div>
          ...
          <div className="bg-blue-500/5 border border-blue-500/10 p-6 rounded-3xl">
              ...
              <div className="flex flex-wrap gap-2">
                  {groundingSources.map((source: any, idx: number) => (
                      <a key={idx} href={source.web?.uri} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 px-3 py-1.5 rounded-lg text-[9px] font-bold hover:bg-blue-500 hover:text-white transition-all">
                          <ExternalLink size={10} /> {source.web?.title || 'Источник'}
                      </a>
                  ))}
              </div>
          </div>
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-1 p-1 bg-black/5 dark:bg-white/5 rounded-2xl border border-black/5 dark:border-white/5 backdrop-blur-3xl sticky top-6 z-30 shadow-2xl">
          {[
            ...
            { id: 'seo', label: 'Рынок', icon: Search }
              ...
              <button 
                ...
                className={`flex-1 flex items-center justify-center gap-2 px-2 py-2.5 rounded-xl font-bold text-[8px] uppercase tracking-widest transition-all ${activeTab === tab.id ? 'bg-white dark:bg-white/20 text-black dark:text-white shadow-md scale-[1.01]' : 'opacity-40 hover:opacity-100'}`}
              >
      ...
      <div className="animate-fade-in min-h-[400px]">
          {activeTab === 'diagnostic' && (
              <div className="bg-white dark:bg-white/5 p-8 rounded-5xl border border-black/5 dark:border-white/10">
                  <h3 className="text-xl font-bold heading-refined mb-8 flex items-center gap-3">
                      ...
                      {(Array.isArray(idea.diagnostic?.groups) && idea.diagnostic!.groups.length > 0) ? (
                        idea.diagnostic!.groups.map(group => (
                          <div key={group.id} className="p-6 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5">
                              <h4 className="font-bold uppercase text-[9px] tracking-widest opacity-30 mb-5 border-b border-black/5 dark:border-white/5 pb-2">{group.title}</h4>
                              <div className="space-y-4">
                                  {(Array.isArray(group.problems) ? group.problems : []).map((p, i) => (
                                      <div key={i}>
                                          <p className="font-bold text-xs mb-2 opacity-90">{p.issue}</p>
                        ...
                        <div className="col-span-3 py-20 text-center opacity-30 italic text-sm">Данные диагностики отсутствуют</div>
                      )}
                  </div>
              </div>
          )}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {(Array.isArray(idea.microIdeas) && idea.microIdeas.length > 0) ? (
                        idea.microIdeas.map((item, i) => (
                          <div key={i} className="p-6 rounded-3xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 group hover:bg-blue-600 hover:text-white transition-all duration-300">
                              <div className="flex justify-between items-start mb-2">
                                  <h4 className="font-bold text-base heading-refined">{item.title}</h4>
                                  <span className="text-[8px] font-bold bg-white/10 px-1.5 py-0.5 rounded uppercase">{item.priority}</span>
                              </div>
                              <p className="text-xs opacity-60 font-medium mb-3 leading-relaxed group-hover:text-white/80">{item.description}</p>
                              <div className="flex justify-between items-center text-[9px] font-bold uppercase tracking-widest">
                                  <span>Ресурс: {item.hours || '0'}ч</span>
                                  <span>Оценка: {item.cost || 'Расчет...'}</span>
                              </div>
                          </div>
                        ))
                      ) : (
                        <div className="col-span-2 py-20 text-center opacity-30 italic text-sm">Данные о модулях отсутствуют</div>
                      )}
                  </div>
              </div>
          )}
              ...
              <div className="bg-white dark:bg-white/5 p-8 rounded-5xl border border-black/5 dark:border-white/10">
                  ...
                  <div className="space-y-1">
                      {(Array.isArray(idea.roadmap) && idea.roadmap.length > 0) ? (
                          ...
                          <div key={i} className="flex gap-6 relative">
                              ...
                              <div className="pb-8 pt-0.5 flex-1">
                                  <div className="flex items-center gap-3 mb-1">
                                    <h4 className="font-bold text-base heading-refined">{step.phase}</h4>
                                    <span className="text-[9px] font-bold opacity-30 italic">{step.duration}</span>
                                  </div>
                                  <p className="text-xs opacity-50 font-medium mb-2">{step.description}</p>
                                  <div className="flex flex-wrap gap-1">
                                      {(Array.isArray(step.tasks) ? step.tasks : []).map((t, ti) => (
                                          <span key={ti} className="px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/5 text-[8px] font-bold uppercase tracking-wider">{t}</span>
                                      ))}
                                  </div>
                              </div>
                          </div>
                        ...
                        <div className="py-20 text-center opacity-30 italic text-sm">План внедрения не сформирован</div>
                      )}
                  </div>
              </div>
          )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                      <div className="space-y-6">
                          <div>
                            ...
                            <p className="text-base font-medium leading-relaxed italic opacity-80">{idea.investorProposal?.pitch || "Готовим питч..."}</p>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                              <div className="p-5 rounded-3xl bg-green-500/5 border border-green-500/10">
                                  <span className="text-[9px] font-bold uppercase text-green-500 tracking-widest mb-1 block">Для инвестора</span>
                                  <p className="text-xs font-bold">{idea.investorProposal?.investorBenefit || "Расчет выгоды..."}</p>
                              </div>
                              <div className="p-5 rounded-3xl bg-blue-500/5 border border-blue-500/10">
                                  <span className="text-[9px] font-bold uppercase text-blue-500 tracking-widest mb-1 block">Для собственника</span>
                                  <p className="text-xs font-bold">{idea.investorProposal?.initiatorBenefit || "Расчет выгоды..."}</p>
                              </div>
                          </div>
                      </div>
                      <div className="p-6 rounded-4xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5">
                              ...
                              {(Array.isArray(idea.investorProposal?.financialHighlights) && idea.investorProposal!.financialHighlights.length > 0) ? (
                                idea.investorProposal!.financialHighlights.map((fh, i) => (
                                  <div key={i} className="flex items-center gap-2">
                                      ...
                                      <span className="text-[10px] font-medium opacity-70">{fh}</span>
                                  </div>
                                ))
                              ) : (
          ...
          {activeTab === 'seo' && (
              <div className="space-y-6">
                  <div className="bg-black text-white dark:bg-white/5 p-8 rounded-5xl shadow-2xl">
                      <h4 className="text-xl font-bold heading-refined mb-6 tracking-tight">Рыночный спрос (Live)</h4>
                            ...
                            idea.searchQueries.map((q, i) => (
                              <div key={i} className="flex items-center justify-between p-5 rounded-3xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-all">
                                  <div className="flex flex-col">
                                      <span className="text-sm font-bold italic heading-refined">{q.query}</span>
                                      <span className="text-[8px] font-bold uppercase opacity-30 mt-1">Competition: {q.competition}</span>
                                  </div>
                                  <div className="text-right">
                                      <div className="text-[9px] font-bold text-blue-400 uppercase tracking-widest mb-0.5">Vol/mo</div>
                                      <div className="font-bold text-xl tracking-tight text-white">{q.volume}</div>
                                  </div>
                              </div>
                            ))
                          ) : (
                  ...
                  <button 
                    ...
                    className="w-full bg-blue-600 text-white py-6 rounded-full font-bold uppercase tracking-[0.2em] text-base shadow-3xl hover:bg-blue-500 transition-all hover:scale-101 active:scale-95 flex items-center justify-center gap-3"
                  >
